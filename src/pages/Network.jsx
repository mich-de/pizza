        ...
        <div className="lg:col-span-7">
          ...
          <div className="flex flex-col gap-4">
            {(selectedCity ? selectedData : data).map((pz) => (
              <div
                key={pz.id}
                className="bg-surface-bright border-4 border-primary p-4 shadow-[4px_4px_0px_0px_rgba(26,26,26,1)] flex items-center gap-4 hover:-translate-y-0.5 transition-transform"
              >
                ...
                <div className="text-right flex-shrink-0">
                  <div className="font-headline font-black text-xl">€{pz.margheritaPrice?.toFixed(2)}</div>
                  <div className="flex items-center gap-1 justify-end">
                    <span className="material-symbols-outlined text-secondary text-sm" style={{ fontVariationSettings: "'FILL' 1" }}>star</span>
                    <span className="font-label font-bold text-sm">{pz.rating}</span>
                  </div>
                </div>
                <a
